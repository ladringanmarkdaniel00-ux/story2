...
export function EditPostHeader({ isOnline, isUploading, requestDismissal }: EditPostHeaderProps) {
  return (
    <>
      {!isOnline && (
        <div className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-2 flex items-center justify-center gap-2 text-xs text-amber-400">
          ...
          <span>You are currently offline. Saving is temporarily disabled.</span>
        </div>
      )}
      <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-800">
        <h2 id="edit-post-title" className="text-lg font-medium text-neutral-100">
          Edit Post
        </h2>
        <button
          type="button"
